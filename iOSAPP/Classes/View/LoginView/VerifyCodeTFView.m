...
@interface VerifyCodeTFView()<UITextFieldDelegate,GTTextFieldDelegate>
/** */
...
@implementation VerifyCodeTFView
...
- (instancetype)initWithFrame:(CGRect)frame onFinishedEnterCode:(OnFinishedEnterCode)onFinishedEnterCode {
    if (self = [super initWithFrame:frame]) {
        ...
        for (NSInteger i = 0; i<4; i++) {
            ...
            [label setCornerWithRadius:5.f borderWidth:.5 borderColor:[UIColor disabledColor]];
        ...
        [textField setCornerWithRadius:5.f borderWidth:.5 borderColor:[UIColor appBaseColor]];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (!self.label1.text.length) {
        ...
        [self.label1 setCornerWithRadius:5.f borderWidth:.5 borderColor:[UIColor appBaseColor]];
        _codeTextField.left = _label2.left;
    }else if (!self.label2.text.length) {
        self.label2.text = string;
        [self.label2 setCornerWithRadius:5.f borderWidth:.5 borderColor:[UIColor appBaseColor]];
        _codeTextField.left = _label3.left;
    }else if (!self.label3.text.length) {
        self.label3.text = string;
        [self.label3 setCornerWithRadius:5.f borderWidth:.5 borderColor:[UIColor appBaseColor]];
        _codeTextField.left = _label4.left;
    }else if (!self.label4.text.length) {
        self.label4.text = string;
        [self.label4 setCornerWithRadius:5.f borderWidth:.5 borderColor:[UIColor appBaseColor]];
        _codeTextField.left = _label4.left;
