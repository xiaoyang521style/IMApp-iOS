...
+ (CGSize)neededSizeForPhoto:(UIImage *)photo {
    ...
    CGSize photoSize = CGSizeMake(newSize.width, newSize.height);
    return photoSize;
}
...
//    float gapDuration = (!voiceDuration || voiceDuration.length == 0 ? -1 : [voiceDuration floatValue] - 1.0f);
    ...
    float VoicePlayTimes = voiceDuration.length;
    ...
    CGSize voiceSize = CGSizeMake(barLen, 42);
    return voiceSize;
}
...
- (void)layoutSubviews {
    ...
    switch (currentType) {
        ...
        case XHBubbleMessageMediaTypeVideo:
        case XHBubbleMessageMediaTypeLocalPosition: {
            CGSize needPhotoSize;
            if (currentType == XHBubbleMessageMediaTypePhoto) {
                needPhotoSize = [XHMessageBubbleView neededSizeForPhoto:self.message.photo];
            }
            if (currentType == XHBubbleMessageMediaTypeVideo) {
                needPhotoSize = [XHMessageBubbleView neededSizeForPhoto:self.message.videoConverPhoto];
            }
