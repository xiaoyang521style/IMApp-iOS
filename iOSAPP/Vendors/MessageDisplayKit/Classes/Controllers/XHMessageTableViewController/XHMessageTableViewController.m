...
- (XHShareMenuView *)shareMenuView {
    if (!_shareMenuView) {
        XHShareMenuView *shareMenuView = [[XHShareMenuView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight + TabbarSafeBottomMargin)] ;
...
//         emotionManagerView.backgroundColor = [UIColor redColor];
...
- (void)setup {
    ...
    self.keyboardViewHeight = (kIsiPad ? 264 : 216 + TabbarSafeBottomMargin);
...
- (void)initilzer {
    ...
    CGRect inputFrame = CGRectMake(0.0f,
                                   self.view.frame.size.height - inputViewHeight - TabbarSafeBottomMargin,
                                   self.view.frame.size.width,
                                   inputViewHeight + TabbarSafeBottomMargin);
        ...
        if (weakSelf.textViewInputViewType == XHInputViewTypeText) {
            [UIView animateWithDuration:duration
                             ...
                             animations:^{
                                 ...
                                 CGRect inputViewFrame = weakSelf.messageInputView.frame;
                                 CGFloat inputViewFrameY = keyboardY - inputViewFrame.size.height - - TabbarSafeBottomMargin;
...
- (void)didSendMessageWithPhoto:(UIImage *)photo {
    ...
    if ([self.delegate respondsToSelector:@selector(didSendPhoto:originPhotoPath:fromSender:onDate:)]) {
        [self.delegate didSendPhoto:photo originPhotoPath:nil fromSender:self.messageSender onDate:[NSDate date]];
    }
}
