...
- (CAShapeLayer *)bgLayer
{
    if (!_bgLayer) {
        ...
        _bgLayer.fillColor = rgba(91, 142, 234,0.5).CGColor;
    }
    return _bgLayer;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    imgV.frame = CGRectMake(rect.size.width / 2 - imgVH / 2, 100, imgVH, imgVH);
    imgV.image = [UIImage imageNamed:@"IMG_2683.jpg"];
    imgV.layer.cornerRadius = imgVH * 0.5;
    imgV.layer.masksToBounds = YES;
    [self addSubview:imgV];
    UILabel *nameLab = [[UILabel alloc] init];
    nameLab.frame = CGRectMake(0, 190, [UIScreen mainScreen].bounds.size.width, 20);
    nameLab.text = @"mmp";
